        ...
        if (uri != null && uri.length() > 1) {
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                ...
                try {
                    ...
                    Glide.with(context).load(id).crossFade().into(imageView);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            } else if (uri.startsWith(ASSET_PREFIX)) {
                String file = uri.substring(ASSET_PREFIX.length());
                Glide.with(context).load(Uri.parse("file:///android_asset/".concat(file))).crossFade()
                        .signature(new StringSignature(String.valueOf(System.currentTimeMillis()))).into(imageView);
            } else {
                Glide.with(context).load(new File(uri)).crossFade().into(imageView);
            }
        }
    }
