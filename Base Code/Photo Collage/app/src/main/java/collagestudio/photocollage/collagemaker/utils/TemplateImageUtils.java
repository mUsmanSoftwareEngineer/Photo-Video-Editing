...
import collagestudio.photocollage.collagemaker.model.TemplateFirebaseImage;
...
public class TemplateImageUtils {
    ...
    static ArrayList<TemplateFirebaseImage> firebaseImages = new ArrayList<>();
    private static void setupFirebaseStorage(){
...
//        item.setPreview(getURI(templateName+PREVIEW_POSTFIX));
//        item.setTemplate(getURI(templateName+TEMPLATE_POSTFIX));

        PhotoItem photoItem = new PhotoItem();
        photoItem.index = 0;
        photoItem.maskPath = createMaskPath(templateName, 0);//getURI(templateName+"_0.png");
        photoItem.x = 52;
        photoItem.y = 180;
        ...
        photoItem.index = 1;
        photoItem.maskPath = createMaskPath(templateName, 1);//getURI(templateName+"_1.png");
        photoItem.x = 215;
        photoItem.y = 360;
        item.getPhotoItemList().add(photoItem);
...
//        item.setTemplate(getURI(templateName+TEMPLATE_POSTFIX));
    ...
    private static TemplateItem template_1_20() {
        final String templateName = "1_20";
        final TemplateItem item = template(templateName);
//        TemplateItem item = new TemplateItem();
//        item.setPreview(getURI(templateName+PREVIEW_POSTFIX));
//        item.setTemplate(getURI(templateName+TEMPLATE_POSTFIX));
...
//        item.setTemplate(getURI(templateName+TEMPLATE_POSTFIX));
    ...
    private static TemplateItem template_0_21() {
        final String templateName = "0_21";
        final TemplateItem item = template(templateName);
//        TemplateItem item = new TemplateItem();
//        item.setPreview(getURI(templateName+PREVIEW_POSTFIX));
//        item.setTemplate(getURI(templateName+TEMPLATE_POSTFIX));
...
//        item.setTemplate(getURI(templateName+TEMPLATE_POSTFIX));
    ...
    public static ArrayList<TemplateItem> loadTemplates(Context context) {

        //storeFirebaseImageToSharedPref(firebaseImages,context);
    ...
    static String getURI(String name){
        ...
        for (int i =0;i<firebaseImages.size();i++){
            if (firebaseImages.get(i).getName().equals(name)){
                uri =  firebaseImages.get(i).getUri();
    ...
    public static void storeFirebaseImageToSharedPref(ArrayList<TemplateFirebaseImage> list, Context context){
        SharedPreferences.Editor preferences = PreferenceManager.getDefaultSharedPreferences(context).edit();
        for (int i =0;i<list.size();i++){
            String KEY = "KEY"+i;
            String value = list.get(i).getName()+"!,!"+list.get(i).getUri();
            preferences.putString(KEY,value);
        }
        preferences.putInt("fbCount",list.size());
        Log.d("fbCount", "storeFirebaseImageToSharedPref: "+list.size());
        preferences.apply();
    }
    public static boolean retrieveFirebaseImageFromSharedPref( Context context){
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        firebaseImages.clear();
        firebaseImages = new ArrayList<>();
        int size = preferences.getInt("fbCount",0);
        Log.d("fbCount", "retrieveFirebaseImageFromSharedPref: "+size);
        ...
        if (size>0){
            for (int i =0;i<size;i++){
                ...
                if (!res.equals("")){
                    String vlaues[] = res.split("!,!");
                    TemplateFirebaseImage image = new TemplateFirebaseImage(vlaues[0],vlaues[1]);
